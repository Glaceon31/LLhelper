...
def llunit():
    ...
    if  request.form.has_key('submit'):
        ...
        result['bonusatt'][mapcenter['bonus2']] += mapcenter['percentage2']*result['baseatt'][mapcenter['base2']]/100.0
        ...
        result['minscore'] = 0
        for i in range(0, 9):    
            result['minscore'] += int(showatt/80.0*member[i]['weight']*combomulti*accmulti*(1+0.1*(mapcenter['map'] == member[i]['main'])))
        #
        ...
        for i in range(0, 9):
            ...
            if skill == 1 or skill == 11:
                ...
                averageskillscore[i] = mapcenter['combo']/member[i]['require']*member[i]['possibility']*member[i]['score']/100
                result['averagescore'] += mapcenter['combo']/member[i]['require']*member[i]['possibility']*member[i]['score']/100 
            if skill == 2:
                skillchance[i] = mapcenter['perfect']/member[i]['require']
                result['maxscore'] += mapcenter['perfect']/member[i]['require']*member[i]['score']
                averageskillscore[i] = mapcenter['perfect']/member[i]['require']*member[i]['possibility']*member[i]['score']/100
                result['averagescore'] += mapcenter['perfect']/member[i]['require']*member[i]['possibility']*member[i]['score']/100
            if skill == 4:
                skillchance[i] = mapcenter['time']/member[i]['require']
                result['maxscore'] += mapcenter['time']/member[i]['require']*member[i]['score']
                averageskillscore[i] = mapcenter['time']/member[i]['require']*member[i]['possibility']*member[i]['score']/100
                result['averagescore'] += mapcenter['time']/member[i]['require']*member[i]['possibility']*member[i]['score']/100
            if skill == 7 or skill == 13:
                skillchance[i] = mapcenter['combo']/member[i]['require']
                result['maxheal'] += mapcenter['combo']/member[i]['require']*member[i]['score']
                result['averageheal'] += mapcenter['combo']/member[i]['require']*member[i]['possibility']*member[i]['score']/100
            if skill == 8:
                skillchance[i] = mapcenter['time']/member[i]['require']
                result['maxheal'] += mapcenter['time']/member[i]['require']*member[i]['score']
                result['averageheal'] += mapcenter['time']/member[i]['require']*member[i]['possibility']*member[i]['score']/100
            if skill == 9:
                skillchance[i] = mapcenter['perfect']/member[i]['require']
                result['maxheal'] += mapcenter['perfect']/member[i]['require']*member[i]['score']
                result['averageheal'] += mapcenter['perfect']/member[i]['require']*member[i]['possibility']*member[i]['score']/100
            if skill == 10:
                ...
                averageskillscore[i] = mapcenter['starperfect']/member[i]['require']*member[i]['possibility']*member[i]['score']/100
                result['averagescore'] += mapcenter['starperfect']/member[i]['require']*member[i]['possibility']*member[i]['score']/100
        #score scoring
        finish = False
        infinite = False
        ...
        while not finish:
            finish = True
            for i in range(0, 9):
                if member[i]['skill'] == 3 and averagescoringtimes[i] < int(result['averagescore']/member[i]['require']):
        ...
        if result['averagescore'] != 'impossible':
            ...
            for i in range(0, 9):
                ...
                if member[i]['main'] != mapcenter['map']:
                    result['unitstrength'] -= int(showatt*member[i]['weight']/11.0/mapcenter['combo']/(1+0.0025*mapcenter['slider']))
                    result['singlestrength'][i] -= int(showatt*member[i]['weight']/11.0/mapcenter['combo']/(1+0.0025*mapcenter['slider']))
                else:
                    result['singlestrength'][i] += member[i]['kizuna']
                result['singlestrength'][i] = int(result['singlestrength'][i])
        #print simresult
        result['simresult'] = {1:simresult[times/100], 2:simresult[times/50], 5:simresult[times/20], 10:simresult[times/10],\
                                20:simresult[times/5], 30:simresult[times*3/10], 40:simresult[times*4/10], 50:simresult[times/2],\
                                60:simresult[times*6/10], 70:simresult[times*7/10], 80:simresult[times*8/10], 90:simresult[times*9/10],\
