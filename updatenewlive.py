...
if __name__ == "__main__":
	...
	while jptmp:
		...
		if jptmp[2] == 1:
			...
			song['niji'] = 1
		if not song.has_key('totaltime') or song['totaltime'] == 0:
		...
		while livetmp:
			...
			# diff is deprecated, will use settings which allow multiple live with same difficulty
			if not song.has_key('settings'):
				song['settings'] = {}
			if not song['settings'].has_key(liveSettingId):
